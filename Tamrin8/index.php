<?php
    ...
    if (isset($_POST["post_comment"]))
    {
        ...
        mysqli_query($conn, "INSERT INTO comment(name, email, comment, post_id, created_at, reply_of) VALUES ('" . $name . "', '" . $email . "', '" . $comment . "', '" . $post_id . "', NOW(), '" . $reply_of . "')");
    ...
    $result = mysqli_query($conn, "SELECT * FROM comment WHERE post_id = " . $post_id);
    ...
    foreach ($comment as $comment_key => $comment)
    {
        ...
        if ($comment->reply_of == 0)
        {
            ...
            foreach ($comment as $reply_key => $reply)
            {
